"""
Finalization pass on the restricted clinical/integrated models. Two changes
from the models behind final_metrics_by_threshold.csv / delong_results.csv:

  1. kidney_transplant added to RESTRICTED_DROP_COLS (build_integrated_model.py) --
     98.4% missing, essentially all imputation, no real signal. Restricted
     clinical feature count: 117 -> 116. Restricted integrated: 117 (116
     clinical + 1 imaging-derived predicted_los), same total as before by
     coincidence (117 either way) but for a different reason -- previously
     117 clinical + 1 imaging = 118; now 116 clinical + 1 imaging = 117.
  2. Integrated-restricted now uses the SAME tuned XGBoost config as
     clinical-only-restricted (eta=0.03, max_depth=3, min_child_weight=5,
     num_boost_round=5000, early_stopping_rounds=100) -- previously it used
     the untuned default config, an inconsistent asymmetry.

Full-set models (clinical_only_full, integrated_full) are NOT touched --
still on the original default config, original 128/129-feature sets. Not
regenerated by this script.

No new data, no imaging retraining -- imaging OOF predictions
(oof_imaging_only.csv) are reused as-is.

Writes oof_clinical_only_restricted_final.csv and
oof_integrated_restricted_final.csv. Does NOT overwrite
oof_clinical_only_restricted.csv, oof_clinical_only_restricted_tuned.csv, or
oof_integrated_restricted.csv (the pre-finalization files) -- those stay on
disk unchanged for before/after comparison.
"""
import os
import numpy as np
import pandas as pd
...
from step4_delong import TUNED_PARAMS, NUM_BOOST_ROUND, EARLY_STOPPING_ROUNDS  # eta=0.03, depth=3, mcw=5, 5000/100
...
def main():
    print("=" * 70)
    print("FINALIZATION: kidney_transplant dropped + integrated-restricted tuned")
    print("=" * 70)
